...
const EditCourse = (props) => {
  ...
  const [courseType, setCourseType] = React.useState(
    course.place.virtual ? "online" : "present"
  );
  ...
  const onFinish = async (values) => {
    // update Place
    let coursePlaceId = course.place.id;
    const isVirtual = courseType == "online";
    if (
      (course.place.virtual && !isVirtual) ||
      (!course.place.virtual && isVirtual)
    ) {
      const updatePlace = {
        id: course.place.id,
  ...
  return (
    <div className="flex flex-col w-full h-full">
      <div className="w-full mt-10 mb-5 text-center border-b">
      ...
      <div className="flex flex-col w-3/4 pl-28">
        ...
        >
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: "Toto pole je povinné!" }]}
          >
            <Select placeholder="Kategorie kurzu">
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: "Toto pole je povinné!" }]}
          >
            <Select placeholder="Obtížnost kurzu">
          ...
          <Form.Item label="Typ" name="type" valuePropName={courseType}>
          ...
          >
            <TextArea
              placeholder="Zde mužete vložit url obrazku kurzu"
              autoSize={true}
            />
          </Form.Item>
